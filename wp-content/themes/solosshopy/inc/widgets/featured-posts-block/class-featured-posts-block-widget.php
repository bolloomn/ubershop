...
if ( ! class_exists( 'Solosshopy_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Solosshopy_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'solosshopy' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'solosshopy' );
			$this->widget_id          = 'solosshopy_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => SOLOSSHOPY_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'solosshopy' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'solosshopy' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'solosshopy' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'solosshopy' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'solosshopy' ),
						'categories' => esc_html__( 'Show categories', 'solosshopy' ),
						'tags'       => esc_html__( 'Show tags', 'solosshopy' ),
						'author'     => esc_html__( 'Show author', 'solosshopy' ),
						'date'       => esc_html__( 'Show date', 'solosshopy' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'solosshopy' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'solosshopy' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = solosshopy_get_locate_template( 'inc/widgets/featured-posts-block/views/widget.php' );
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = solosshopy_get_locate_template( 'inc/widgets/featured-posts-block/views/item.php' );
			 ...
			 */
			$query = apply_filters( 'solosshopy_featured_posts_block_query', $query, $this->instance );
		...
		public function prepare_data( $data, $layout ) {
			...
			$container_template = solosshopy_get_locate_template( 'inc/widgets/featured-posts-block/views/container.php' );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'solosshopy' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'solosshopy' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'solosshopy' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'solosshopy' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'solosshopy' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'solosshopy_register_featured_posts_block_widget' );

	if ( false === function_exists( 'solosshopy_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function solosshopy_register_featured_posts_block_widget() {
			register_widget( 'Solosshopy_Featured_Posts_Block_Widget' );
		}
	}
}
