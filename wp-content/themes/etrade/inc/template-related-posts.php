 ...
 * @return array
 */
function solosshopy_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = get_theme_mod( 'related_posts_visible', solosshopy_theme()->customizer->get_default( 'related_posts_visible' ) );
	...
	$post_terms  = array();
	$post_number = get_theme_mod( 'related_posts_count', solosshopy_theme()->customizer->get_default( 'related_posts_count' ) );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = get_theme_mod( $setting_value, solosshopy_theme()->customizer->get_default( $setting_value ) );
	}

	$settings['title_visible']   = $settings['title_length'] ? get_theme_mod( 'related_posts_title', solosshopy_theme()->customizer->get_default( 'related_posts_title' ) ) : false;
	...
	if ( $holder_view_dir ) {
		...
		$block_title_format = apply_filters( 'solosshopy_rp_block_title_format', '<h5 class="related-posts__title">%s</h5>' );
		$block_title        = ( $settings['block_title'] ) ? sprintf( $block_title_format, $settings['block_title'] ) : '';
		$accent_color       = get_theme_mod( 'regular_accent_color_1', solosshopy_theme()->customizer->get_default( 'regular_accent_color_1' ) );
		...
		foreach ( $posts as $post ) {
			...
			$image = $utility->media->get_image( apply_filters( 'solosshopy_rp_image_settings', array(
			...
			$title = $utility->attributes->get_title( apply_filters( 'solosshopy_rp_title_settings', array(
			...
			$excerpt = $utility->attributes->get_content( apply_filters( 'solosshopy_rp_excerpt_settings', array(
				'visible'      => $settings['content_visible'],
				'length'       => $settings['content_length'],
				'content_type' => $settings['content_type'],
			) ) );

			$category = $utility->meta_data->get_terms( apply_filters( 'solosshopy_rp_category_settings', array(
				'type'    => 'category',
			...
			$tag = $utility->meta_data->get_terms( apply_filters( 'solosshopy_rp_tag_settings', array(
				'type'      => 'post_tag',
				'visible'   => $settings['tag_visible'],
				'delimiter' => '<span class="post__tags-delimiter">, </span>',
				'prefix'    => esc_html__( 'Tag: ', 'solosshopy' ),
				'before'    => '<span class="post__tags">',
				'after'     => '</span>',
			) ) );

			$author = $utility->meta_data->get_author( apply_filters( 'solosshopy_rp_author_settings', array(
			...
			$date = $utility->meta_data->get_date( apply_filters( 'solosshopy_rp_date_settings', array(
			...
			$comment_count = $utility->meta_data->get_comment_count( apply_filters( 'solosshopy_rp_comment_count_settings', array(
