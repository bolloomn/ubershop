	...
	class PukkaSocialMedia {
		...
		*/
		...
		public function printSocialButtons() {
			...
			if( is_page() ){
				$page_share_enabled = get_post_meta($post->ID, PUKKA_POSTMETA_PREFIX . 'enable_share', true) == 'on' ? 'on' : 'off';
				
				if( pukka_get_option('page_disable_share') == 'on' && $page_share_enabled != 'on' ){
					return;
				}
			}
