...
if ( ! class_exists( 'Solosshopy_Custom_Posts_Widget' ) ) {

	/**
	 * Class Solosshopy_Custom_Posts_Widget.
	 */
	class Solosshopy_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'solosshopy' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'solosshopy' );
			$this->widget_id          = apply_filters( 'solosshopy_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'solosshopy_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = solosshopy_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'solosshopy' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'solosshopy' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'solosshopy' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'solosshopy' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'solosshopy' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'solosshopy' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'solosshopy' ),
						'category'      => esc_html__( 'Category', 'solosshopy' ),
						'post_tag'      => esc_html__( 'Tag', 'solosshopy' ),
						'more_button'   => esc_html__( 'More Button', 'solosshopy' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'solosshopy' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = solosshopy_get_locate_template( 'inc/widgets/custom-posts/views/custom-post-view.php' );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', solosshopy_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'solosshopy-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'solosshopy_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function solosshopy_register_custom_posts_widget() {
		register_widget( 'Solosshopy_Custom_Posts_Widget' );
	}
}
