 ...
 */
add_filter( 'template_include', array( 'Solosshopy_Wrapping', 'wrap' ), 99 );
add_filter( 'solosshopy_wrap_base', 'solosshopy_wrap_base_cpts' );
 ...
 */
function solosshopy_template_path() {
	return Solosshopy_Wrapping::$main_template;
}
 ...
 */
function solosshopy_template_base() {
	return Solosshopy_Wrapping::$base;
}

if ( ! class_exists( 'Solosshopy_Wrapping' ) ) {
	...
	class Solosshopy_Wrapping {
		...
		/**
		 * Initialize new instance of Solosshopy_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __construct( $template = 'base.php' ) {
			$pathinfo = str_replace( '.php', '', pathinfo( $template, PATHINFO_BASENAME ) );
			$this->slug      = $pathinfo;
		...
		public function __toString() {
			$this->templates = apply_filters( 'solosshopy_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Solosshopy_Wrapping class.
		 *
		 * @since 1.0.0
		...
		public static function wrap( $main ) {
			...
			$pathinfo = str_replace( '.php', '', pathinfo( self::$main_template, PATHINFO_BASENAME ) );
			self::$base = $pathinfo;
			...
			return new Solosshopy_Wrapping();
		}
	}
}
 ...
 */
function solosshopy_wrap_base_cpts( $templates ) {
