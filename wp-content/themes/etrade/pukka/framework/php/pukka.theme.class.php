	...
	class PukkaTheme {
		...
		public function __construct($theme_option_pages) {
			...
			if (is_admin()) {
				...
				//$this->addThemeStyle(); // moved to generate_theme_css() functions
			}
		...
		*/
		...
		public function saveMenuPage() {
			...
			// generate theme css from selected options and write it to file
			$this->update_theme_css();
		...
		/**
		 * Fetch generated theme css and write it to file
		 *
		 * @since Pukka 1.1
		 */
		private function update_theme_css(){
			$theme_css = generate_theme_css(); // function located in pukka/[theme_name]_functions.php
			$uploads_dir = wp_upload_dir();
			file_put_contents ( $uploads_dir['basedir'] . '/theme_style.css' , $theme_css);
		}
		...
		*/
		...
		private function fetchGoogleFonts() {
			...
			$elements = array(
							'title' => '.entry-title, h1, h2, h3, h4, h5, h6', 
							'text' => 'body', 
							'h1' => '.entry-content h1', 
							'h2' => '.entry-content h2', 
							'h3' => '.entry-content h3', 
			...
			foreach($elements as $key => $element){
				...
				if(!empty($font)){
					$url = '//fonts.googleapis.com/css?family=' . $font . ':' . $font_weight;
