...
function woocs_add_fixed_field(post_id,selector, hash) {
    var code = jQuery('#woocs_mselect_'+selector +'_'+ hash).val();
    if (code) {
        var html = jQuery('#woocs_multiple_simple_tpl_'+selector).html();
        html = html.replace(/__POST_ID__/gi, post_id);
        html = html.replace(/__HASH__/gi, hash);
        html = html.replace(/__CURR_CODE__/gi, code);
        jQuery('#woocs_mlist_'+selector +'_' + hash).append(html);
        //***
        jQuery("#woocs_mselect_"+selector +'_' + hash + " option[value='" + code + "']").remove();
        jQuery("#woocs_mselect_"+selector +'_' + hash + " option").eq(0).prop('selected', 'selected');
    }
    woocs_enable_save_variation_changes();
}

function woocs_add_all_fixed_field(post_id,selector, hash) {
    jQuery.each(jQuery("#woocs_mselect_"+selector +"_" + hash + " option"), function (i, code) {
        jQuery("#woocs_mselect_"+selector +"_" + hash + " option[value='" + code + "']").prop('selected', 'selected');
...
function woocs_remove_li_fixed_field(hash, code, geo,selector) {
    ...
    if (geo) {
        ...
        jQuery('#woocs_mlist_'+selector +'_' + hash + ' #woocs_li_' + hash + '_' + code).remove();
    }
